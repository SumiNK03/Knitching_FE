    ...
    if (!pattern) {
        return (
            <div className="p-10">
                ...
                <p className="text-center text-gray-500">패턴 정보를 불러올 수 없습니다.</p>
            </div>
        );
    }
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-[20px] shadow-md p-8">
                            ...
                            <div className="space-y-4 border-t border-[#E0D9CF] pt-6">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-2xl font-black text-[#D18063]">₩{pattern.price?.toLocaleString() || '10,000'}</span>
                                </div>

                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-bold text-[#7A7265]">작가</span>
                                    <span className="text-lg font-semibold text-[#4A3E3D]">{pattern.author}</span>
                                </div>
                                ...
                                {pattern.requiredLevel && (
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm font-bold text-[#7A7265]">권장 숙련도</span>
                                        <span className="text-base font-medium text-[#4A3E3D]">{pattern.requiredLevel}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-[20px] shadow-md p-8">
                            ...
                            <p className="text-base text-[#4A3E3D] leading-relaxed whitespace-pre-wrap">
                                {pattern.description || '이 패턴은 초보자부터 숙련된 뜨개꾼까지 즐길 수 있는 매력적인 디자인입니다. 세밀한 기법과 정교한 마무리로 완성도 높은 작품을 만들 수 있습니다.'}
                            </p>
                        </div>
                        ...
                        <div className="bg-white rounded-[20px] shadow-md p-8">
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1">
                                    <p className="font-bold text-[#4A3E3D] mb-2">{pattern.author}</p>
                        ...
                        <div className="bg-white rounded-[20px] shadow-md p-8">
                            ...
                            <div className="grid grid-cols-3 gap-4 text-center">
                                <div>
                                    <p className="text-3xl font-black text-[#D18063] mb-2">{pattern.views || 1205}</p>
                                    <p className="text-xs font-bold text-[#7A7265]">조회수</p>
                                </div>
                                <div>
                                    <p className="text-3xl font-black text-[#D18063] mb-2">{pattern.enrolls || 342}</p>
                                    <p className="text-xs font-bold text-[#7A7265]">수강신청</p>
                                </div>
                                <div>
                                    <p className="text-3xl font-black text-[#D18063] mb-2">{pattern.rating || '4.8'}</p>
